...
# Initialize session with SQLAlchemy
Session(app)
...
def create_tables():
    with get_db() as conn:
        with conn.cursor() as cur:
            # Create telethon_sessions table
            cur.execute("""
...
def login():
    try:
        ...
        if session.get('logged_in') and session.get('session_string') and session.get('user_phone'):
...
async def dashboard():
    try:
        ...
        if not all([session.get('logged_in'), session.get('session_string'), session.get('user_phone')]):
    ...
    try:
        ...
        if status:
            try:
                ...
                os.environ['PHONE_NUMBER'] = user_phone
                os.environ['SESSION_STRING'] = session_string
                os.environ['SOURCE_CHANNEL'] = source
                os.environ['DESTINATION_CHANNEL'] = destination
                ...
                if session.get('bot_running'):
                    ...
                    os.environ.pop('DESTINATION_CHANNEL', None)
                    EventLoopManager.reset()
            ...
            try:
                # Stop bot by clearing environment variables
                os.environ.pop('PHONE_NUMBER', None)
                os.environ.pop('SESSION_STRING', None)
                os.environ.pop('SOURCE_CHANNEL', None)
                os.environ.pop('DESTINATION_CHANNEL', None)
